          ...
          <div class="dataTable">
            <el-table ref="multipleTable" border stripe :data="tableData3" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" @row-click="tableRowCLick">
              ...
              <el-table-column label="类型" width="100">
                <template slot-scope="scope">{{ scope.row.type == '01' ? '运营人员':'服务人员' }}</template>
              </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      afterComfirmDelete: async function(){
        ...
        const resData = await axios.delete('api/users?accesstoken='+this.token,data)
        if(resData.code == '200'){
          this.$message('删除成功')
          this.loadData();
      ...
      loadData: async function(page = 1){
        ...
        this.selectIndex = 0
        const resData = await axios.post('/api/users/?accesstoken='+this.token,data)
        this.tableData3 = resData.data
